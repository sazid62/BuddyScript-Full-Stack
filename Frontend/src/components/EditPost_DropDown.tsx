...
//       })
//     );
export default function EditPost_DropDown({
  newText,
  handleEditSubmit,
  profile,
}) {
  const [isOpen, setIsOpen] = useState(true);
  const [postText, setPostText] = useState(newText || "");
  const [currentUser, setCurrentUser] = useState({ img: profile });
            ...
            <div className="p-4">
              <div className="mb-4 flex items-start gap-3">
                <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0">
                  {currentUser.img ? (
                    <img
                      src={currentUser.img || "/placeholder.svg"}
                      alt="User"
                ...
                <div className="form-floating w-full relative">
                  <textarea
                    required
                    value={postText}
                    onChange={(e) => {
                    ...
                    className="w-full border rounded-lg p-3 min-h-[120px] focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                    placeholder=""
                    id="floatingTextarea"
                  ></textarea>
                  {postText === "" && (
                    <div className="absolute top-3 left-3 flex flex-row gap-2 text-gray-500 pointer-events-none">
              ...
              <div className="flex justify-between mt-4">
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-2 px-4 py-2 bg-blue-500 text-white rounded-md transition-all ${
                ...
                >
                  <svg
                    ...
                    fill="none"
                    viewBox="0 0 14 13"
                    className={`${isSubmitting ? "animate-send" : ""}`}
                  >
                    <path
                      fill="#fff"
                      fillRule="evenodd"
